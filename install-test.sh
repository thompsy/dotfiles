...
install_fonts() {
    ...
    for font in "${FONT_PACKAGES[@]}"; do
        ...
        if command -v fc-list >/dev/null 2>&1 && fc-list | grep -qi "$font"; then
            ...
            # Run nerd-installer for this specific font
            if "$installer_dir/install.sh" "$font" 2>/dev/null; then
                log_success "$font installed successfully"
            else
                log_warning "Failed to install $font - continuing with other fonts"
            fi
        fi
    done
...
install_emacs() {
    ...
    log_info "Skipping Emacs build for test - would take 30+ minutes"
    log_success "Emacs installation skipped"
}
